...
int main ()
{
    ...
    for(p = 0; p < numPlayer; p++) 
    {
        ...
        G.hand[p][idx] = village;  // swap card at position "idx" in player's hand to a smithy

        my_assert(cardEffect(village, 0, 0, 0, &G, idx, &coin_bonus) == 0); // smithy didn't fail
    #if VERBOSE
        ...
        my_assert(G.discard[p][0] == village); // discarded card is smithy
        my_assert(G.handCount[p] == 5);    // 5 cards in hand (5-1+1)
        my_assert(G.numActions == (p+2));    // incremented num of actions
    }
    printf("END OF TEST\n");
    printf("-------------\n\n");;
    return 0;
