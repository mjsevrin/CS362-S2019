...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  case feast:
    return feast_effect(state, choice1, currentPlayer);
...
int adventurer_effect(struct gameState* state, int player)
{
  ...
  {
    if (state->deckCount[currentPlayer] > 1) // introduced bug here, should be < 1
...
int council_room_effect(struct gameState* state, int player, int handPos)
{
  int i;
  ...
  for (i; i < state->numPlayers; i++) // BUG HERE, should be "i = 0" otherwise i = 4 from previous loop
  {
    if (i != player)
    {
      drawCard(i, state);
...
int feast_effect(struct gameState* state, int choice1, int player)
{
  int i;
  int temphand[MAX_HAND];
...
int smithy_effect(struct gameState* state, int player, int handPos)
{
  int i;
  ...
  for (i = 0; i < 3; i++)
  {
    drawCard(player, state);
    drawCard(player, state);
    drawCard(player, state); // BUG: since you have a for loop you should not call drawCard 3 times
  }
...
int village_effect(struct gameState* state, int player, int handPos)
{
  //+1 Card
  drawCard(player, state);
  ...
  //discard played card from hand
  discardCard(handPos, player, state, 1); // BUG: Thrashflag set to 1, thrashing instead of discarding
  return 0;
}
