...
int main ()
{
  srand(time(0));
    ...
    while(testCount < 100)
    {
        ...
        G.hand[p][i] = village;  // swap card at ith position in player's hand to an adventurer
        ...
        int function_return = cardEffect(village, 0, 0, 0, &G, i, &coin_bonus);
        ...
        if(!assertTrue(G.handCount[p] == handCounter, "Incorrect # of cards in hand")) // check hand count (-1 great hall, +1 drawn card)
            handCountFailures++;
        if(!assertTrue(G.numActions == (oldNumActions + 2), "Incorrect # of actions left")) // gain one action
            actionCountFailures++;
    }
